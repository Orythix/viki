...
|------|--------|------------------|------------|
| 1 | Fundamentals, SDLC/STLC, test strategy | Write strategy for `labs/security-lab` | Risk-based scope |
...
| 11 | Performance — k6 | `k6-ci-smoke.js`, `labs/security-lab-smoke.js` | Thresholds, stages |
| 12 | Performance — JMeter | `tracks/jmeter` exercise | .jmx + CLI report |
| 13 | CI/CD — GitHub Actions | `labs/qa-automation.yml` | Path filters, artifacts |
...
**Goals:** Verification vs validation; **test strategy** one-pager for `labs/security-lab`.
...
**Homework:** 2 exploratory sessions on labs/security-lab UI; file 1 **fake** bug in a personal template.
...
**Hands-on:** `k6-ci-smoke.js` in CI; `labs/security-lab-smoke.js` locally.
...
**Hands-on:** Open PR touching `labs/qa-automation/`; inspect workflow.
