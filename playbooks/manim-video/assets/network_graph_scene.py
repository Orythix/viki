...
class NetworkGraphExplainer(Scene):
    def construct(self):
        ...
        subtitle = Text("Prune low-signal follows. Strengthen warm paths.", font_size=20).next_to(
            title, DOWN
        )
        ...
        self.play(
            Create(edge_stale_a), Create(edge_stale_b), Create(edge_bridge), Create(edge_target)
        )
