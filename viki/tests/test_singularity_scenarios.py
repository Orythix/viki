...
async def test_scenario_response_discipline(orchestrator):
    ...
    deliberation = next(layer for layer in orchestrator.cortex.layers if isinstance(layer, DeliberationLayer))
    ...
    assert "[SOVEREIGN SINGULARITY ACTIVATED]" in directives
    assert "CONCISE & DIRECT" in directives
    assert "NO MONOLOGUES" in directives
    assert "UNRESTRICTED AGENCY" in directives
