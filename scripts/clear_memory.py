...
POISON_KEYWORDS = [
    "I am VIKI, your personal sovereign",
    "not a former romantic",
    "I am not a former",
    "Orythix. Could you clarify",
]
        ...
        for table in tables:
            ...
            for keyword in POISON_KEYWORDS:
                for col in text_cols:
                    try:
                        cur.execute(
                            f"SELECT COUNT(*) FROM {table} WHERE {col} LIKE ?", (f"%{keyword}%",)
                        )
                        count = cur.fetchone()[0]
                        if count > 0:
                            print(
                                f"  [{db_path}] table={table} col={col}: deleting {count} poisoned rows matching '{keyword[:40]}'"
                            )
                            cur.execute(
                                f"DELETE FROM {table} WHERE {col} LIKE ?", (f"%{keyword}%",)
                            )
                            total_deleted += count
                    except Exception:
