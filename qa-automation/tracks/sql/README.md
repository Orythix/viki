...
- **Audit / compliance**: row counts, time windows (`audit_log` in security-lab).
...
Same queries adapt to `audit_log` table created by `AuditStore` (see `security-lab/backend/app/audit_store.py`).
