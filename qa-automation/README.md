...
**Parent repository:** [VIKI](../README.md) (this folder lives next to `viki/`, `ui/`, and `security-lab/`).

Hands-on tracks for **API, UI, performance, SQL, mobile patterns**, plus **CI samples** (GitHub Actions, Azure DevOps, Jenkins). Primary API target: **`security-lab`** (local, defensive).
...
**Python live** (API must be running — see `security-lab` README):
...
```powershell
cd qa-automation\tracks\java-api
mvn -q test
```
...
- **GitHub:** `.github/workflows/qa-automation.yml` runs Python unit, Java, k6; Playwright runs with skips unless you enable UI.
