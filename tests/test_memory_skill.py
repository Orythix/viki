...
def mock_controller():
    ...
    controller.learning.get_lessons = MagicMock(return_value=[
        {"id": "test-1", "source_task": "manual", "text_representation": "Test Fact"}
    ])
    ...
    controller.memory.working.get_all_sessions = MagicMock(return_value=[
        {"session_id": "sess-1", "message_count": 5, "last_active": "2026-05-14"}
    ])
...
async def test_memory_save(mock_controller):
    ...
    result = await skill.execute({"action": "save", "text": "Python is great", "category": "coding"})
