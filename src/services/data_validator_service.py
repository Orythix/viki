...
class ValidationReport:
    ...
    def add_error(self, field: str, source: str, message: str, constraint: str):
        self.is_valid = False
        self.errors.append(
            {"field": field, "source": source, "message": message, "constraint": constraint}
        )
...
class DataValidatorService:
    ...
    def __init__(self):
        viki_logger.info("DataValidatorService initialized and ready to enforce schema rules.")

    def validate(self, data: dict | list[dict] | list[tuple], schema_key: str) -> ValidationReport:
        """
        ...
        if isinstance(data, dict):
            ...
            report.add_error(
                "N/A", "System", "Unsupported data type provided to validator.", "Type Error"
            )

        return report

    def _validate_single_dict(
        self, data: dict[str, Any], schema: dict[str, Any], report: ValidationReport
    ):
                ...
                try:
                    if not constraint_func(value):
                        report.add_error(
                            field,
                            "ConstraintCheck",
                            f"Value '{value}' failed custom validation.",
                            "Custom Constraint Failed",
                        )
                except Exception as e:
                    report.add_warning(
                        field, "ConstraintCheck", f"Error during constraint check: {e}"
                    )
