...
class ShellSkill(BaseSkill):
    ...
    def _classify_command(self, command: str) -> str:
        ...
        if (
            ...
            or "&&" in command
            or "||" in command
            or (command.strip() and "|" in command)
        ):
            return "destructive"
    ...
    async def execute(self, params: dict[str, Any]) -> str:
        ...
        if classification == "forbidden":
            viki_logger.warning(
                f"Shell: Blocked forbidden command pattern: {safe_for_log(command)}"
            )
        ...
        if classification == "destructive" and not skip_confirmation:
            viki_logger.warning(
                f"Shell: Destructive command requires confirmation: {safe_for_log(command)}"
            )
            return (
