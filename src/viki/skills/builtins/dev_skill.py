    ...
    async def execute(self, params: dict[str, Any]) -> str:
        ...
        try:
            ...
            if action == "move_file":
                ...
                if not dest:
                    return "Error: move_file requires 'destination'."
    ...
    def _patch_file(
        ...
        try:
            ...
            if occurrence is not None:
                ...
                new_content = (
                    target.join(parts[:occurrence]) + replacement + target.join(parts[occurrence:])
                )
            else:
            ...
            if self._controller:
                self._controller.track_touched_item("touched_files", path)
            return (
                f"Successfully patched {path} (occurrence={occurrence if occurrence else 'all'})."
            )
        except Exception as e:
