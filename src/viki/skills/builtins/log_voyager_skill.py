...
class LogVoyagerSkill(BaseSkill):
    ...
    def _trace_events(self, category: str | None, query: str, limit: int) -> str:
        ...
        for e in events:
            ...
            lines.append(
                f"[{ts}] [{e['severity']}] [{e['category']}] {e['event_type']}: {e['payload']}"
            )
    ...
    def _analyze_anomalies(self, limit: int) -> str:
        if self._telemetry:
            ...
            if errors:
                ...
                for e in errors:
                    lines.append(
                        f"ERROR in {e['category']}: {e['payload'].get('message', str(e['payload']))}"
                    )
                return "\n".join(lines)
    ...
    def _summarize_health(self) -> str:
        if self._telemetry:
            ...
            if summary["errors"] > 5:
                status = "CRITICAL"
            elif summary["errors"] > 0 or summary["warnings"] > 5:
                status = "DEGRADED"
