...
class CachePilotSkill(BaseSkill):
    ...
    def _get_cache(self):
        ...
        if (
            self._controller
            and hasattr(self._controller, "router")
            and hasattr(self._controller.router, "cache")
        ):
    ...
    def schema(self) -> dict:
        return {
            ...
            "properties": {
                ...
                "query": {"type": "string", "description": "Query text for search, prune, or warm"},
                "response": {"type": "string", "description": "Response text for warm action"},
                "limit": {"type": "integer", "default": 10, "description": "Limit for list action"},
            },
            "required": ["action"],
        }
        ...
        try:
            if action == "stats":
                ...
                if query:
                    ...
                    return (
                        f"Entry for '{query}' removed from cache."
                        if success
                        else "Failed to remove entry."
                    )
                else:
                    success = cache.clear()
                    return (
                        "Entire semantic cache wiped successfully."
                        if success
                        else "Failed to wipe cache."
                    )
