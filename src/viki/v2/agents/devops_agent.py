"""DevOpsAgent — Docker, CI/CD pipelines, infrastructure analysis."""
...
class DevOpsAgent(SpecialistAgent):
    name = "devops"
    description = "Analyzes Docker, CI/CD, and infrastructure configuration"
    domain = "devops"
    ...
    async def analyze(self, context: dict) -> AgentFindings:
        ...
        infra_files = context.get("infra_files", "")
        prompt = (
            f"You are an expert DevOps engineer. "
            f"Analyze the infrastructure and produce findings.\n\n"
            f"Goal: {goal}\n"
            f"Infrastructure files: {infra_files}\n\n"
