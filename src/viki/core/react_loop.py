...
async def run_react_loop(
    ...
    *,
    ...
    if reflex_action_override is not None and controller.skill_registry.get_skill(reflex_action_override.skill_name):
        ...
        if check_res.allowed and controller.safety.validate_action(skill_name, params):
            ...
            if not controller.shadow_mode:
                ...
                if not err and result is not None:
                    ...
                    controller.memory.working.add_message("assistant", reflex_msg, session_id=session_id)
                    return reflex_msg
                if err:
                    try:
    ...
    for react_step in range(max_react_steps):
        ...
        try:
            ...
            viki_resp = None
            if task_type == "coding" and controller.world.state.current_phase == "EXECUTING" and not action_results and not is_continuation:
                ...
                controller.planner.callbacks = controller._get_planner_callbacks(session_id, budget, on_event)
            ...
            if task_type == "coding" and viki_resp:
                ...
                if controller.world.state.execution_started and viki_resp.intent_type in ("planning", "discovery"):
            ...
            if len(controller.internal_trace) > 10: controller.internal_trace.pop(0)
        ...
        if viki_resp.action:
            ...
            controller.history.take_snapshot("ACTION_START", f"Executing {skill_name}", {"params": params})
            ...
            if contract_err:
                ...
                controller._last_response_meta_by_session[session_id] = {"contract_error": contract_err}
            ...
            if err:
                ...
                controller.world.state.retry_count += 1
                viki_logger.info(f"FSM: Tool failure detected. Retry count: {controller.world.state.retry_count}")
            ...
            if len(action_results) >= 2:
                ...
                if act0 == act1 and no_result:
                    ...
                    controller._last_response_meta_by_session[session_id] = {"subtasks": action_results, "total_steps": react_step + 1}
                    break
            if react_step < max_react_steps - 1:
                continue
            controller.last_interaction_time = time.time()
            controller._last_response_meta_by_session[session_id] = {"subtasks": action_results, "total_steps": max_react_steps}
    ...
    try:
        ...
        controller._create_tracked_task(controller.learning.analyze_session(controller.model_router.get_model(["reasoning"]), controller.memory.working.get_trace(session_id=session_id), (final_output or "")[:200]), "session_learning")
        ...
        try:
            ...
            if count > 0 and count % 20 == 0:
                controller._create_tracked_task(controller.memory.episodic.consolidate(controller.model_router), "memory_consolidation")
