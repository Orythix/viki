...
def _build_env_overrides() -> dict[str, object]:
    ...
    if os.environ.get("VIKI_WORKSPACE_DIR"):
        overrides["workspace_dir"] = os.path.abspath(os.path.expanduser(os.environ["VIKI_WORKSPACE_DIR"]))
    ...
    if os.environ.get("VIKI_LOCAL_LLM_ONLY") is not None:
        overrides["local_llm_only"] = os.environ.get("VIKI_LOCAL_LLM_ONLY", "").strip().lower() in ("1", "true", "yes")
    ...
    if os.environ.get("VIKI_SESSION_USAGE_LOG") is not None:
        overrides["session_usage_log"] = os.environ.get("VIKI_SESSION_USAGE_LOG", "").strip().lower() in ("1", "true", "yes")
    if os.environ.get("VIKI_AUTO_WEB_RESEARCH") is not None:
        overrides["auto_web_research_when_uncertain"] = os.environ.get("VIKI_AUTO_WEB_RESEARCH", "").strip().lower() in ("1", "true", "yes", "on")
    if os.environ.get("VIKI_BIO_WEBCAM") is not None:
        overrides["bio_webcam_enabled"] = os.environ.get("VIKI_BIO_WEBCAM", "").strip().lower() in ("1", "true", "yes")
