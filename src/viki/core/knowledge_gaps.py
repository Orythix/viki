...
class KnowledgeGapDetector:
    ...
    def record_low_confidence(self, query: str, confidence: float):
        ...
        if confidence < 0.4:
            self.low_confidence_queries.append(
                {"query": query, "confidence": confidence, "timestamp": time.time()}
            )
            ...
            viki_logger.debug(
                f"KnowledgeGap: Recorded low-confidence query: {query[:50]} (conf: {confidence:.2f})"
            )
    ...
    def get_research_topics(self, limit: int = 5) -> list[str]:
        ...
        viki_logger.info(
            f"KnowledgeGap: Generated {len(topics)} research topics from {len(clusters)} clusters"
        )
        return topics
    ...
    def get_gap_summary(self) -> dict[str, Any]:
        ...
        if not self.low_confidence_queries:
            return {"total_gaps": 0, "clusters": 0, "avg_confidence": 0.0}

        clusters = self._cluster_queries(self.low_confidence_queries)
        avg_conf = sum(q["confidence"] for q in self.low_confidence_queries) / len(
            self.low_confidence_queries
        )
