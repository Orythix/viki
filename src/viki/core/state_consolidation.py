...
class DreamModule:
    ...
    async def _spontaneous_cognition(self):
        ...
        if os.environ.get("VIKI_QUIET_BACKGROUND", "").lower() in ("true", "1", "yes"):
            return
        ...
        try:
            ...
            thought_prompt = [
                ...
                {
                    "role": "user",
                    "content": f"You are idle. Generate a brief internal monologue about: {theme}. Speak as yourself—VIKI, the Virtual Intelligence Knowledge Integrator.",
                },
            ]
            ...
            try:
                ...
                if research_skill:
                    ...
                    await research_skill.execute(
                        {"query": f"{topic} overview and latest facts", "num_results": 5}
                    )
    ...
    async def _trigger_self_evolution(self):
        ...
        if new_lessons_count >= 10:
            viki_logger.info(
                f"Dreaming: {new_lessons_count} new lessons detected. Triggering Forge."
            )
