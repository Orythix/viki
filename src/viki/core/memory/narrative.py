...
def _coerce_flat_embedding(v: Any, dim: int) -> list[float] | None:
...
class NarrativeMemory:
    ...
    def __init__(self, data_dir: str, db_path: str | None = None):
    ...
    def add_episode(
        self, context: str, intent: str, plan: dict, action: str, outcome: str, confidence: float
    ):
        ...
        if self.encoder:
            try:
                ...
                story = (
                    f"Context: {context} | Intent: {intent} | Action: {action} | Outcome: {outcome}"
                )
    ...
    def retrieve_context(
        self, current_intent: str, limit: int = 3, cheap: bool = False
    ) -> list[dict[str, Any]]:
        """
    ...
    def _get_recent_episodes(self, limit: int):
        cur = self.conn.cursor()
        cur.execute(
            "SELECT intent, action, outcome FROM episodes ORDER BY timestamp DESC LIMIT ?", (limit,)
        )
    ...
    def decay_memories(self, retention_days: int = 60):
        ...
        self.conn.execute(
            "DELETE FROM episodes WHERE last_accessed < ? AND access_count < 3", (cutoff,)
        )
        self.conn.commit()
    ...
    def get_semantic_knowledge(self, limit: int = 5) -> list[dict[str, Any]]:
        ...
        try:
            ...
            cur = self.conn.cursor()
        cur.execute(
            "SELECT category, insight FROM semantic_knowledge ORDER BY last_reinforced DESC LIMIT ?",
            (limit,),
        )
        return [dict(r) for r in cur.fetchall()]
    ...
    async def consolidate(self, model_router=None):
        ...
        cur = self.conn.cursor()
        cur.execute(
            "SELECT id, intent, outcome FROM episodes WHERE access_count > 0 ORDER BY timestamp DESC LIMIT 20"
        )
        rows = cur.fetchall()
        ...
        try:
            ...
            for line in resp.split("\n"):
                if ":" in line:
                    ...
                    self.conn.execute(
                        """INSERT INTO semantic_knowledge (key, category, insight, source_count, last_reinforced)
                           VALUES (?, ?, ?, ?, ?)
