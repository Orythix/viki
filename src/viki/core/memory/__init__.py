...
from .narrative import NarrativeMemory as EpisodicMemory
...
class WorkingMemory:
    ...
    def _prune_history(self, session_id: str | None = None):
        ...
        with self._lock:
            ...
            try:
                ...
                self.db["messages"].delete_where(
                    "session_id = ? AND timestamp < ?", [session_id, oldest_ts]
                )
            except Exception as e:
                viki_logger.error(f"WorkingMemory Pruning Failed: {e}")

    def get_trace(self, session_id: str | None = None) -> list[dict[str, str]]:
        with self._lock:
            session_id = self._normalize_session_id(session_id)
            if self.db:
                rows = list(
    ...
    def replace_trace(self, messages: list[dict[str, str]], session_id: str | None = None) -> None:
    ...
    def clear_trace(self, session_id: str | None = None) -> None:
        with self._lock:
            session_id = self._normalize_session_id(session_id)
            if self.db:
                self.db["messages"].delete_where("session_id = ?", [session_id])
    ...
    def get_last_thought(self, session_id: str | None = None) -> str:
...
class HierarchicalMemory:
    ...
    def get_context(
        self, current_input: str = "", limit: int = 20, session_id: str | None = None
    ) -> dict[str, Any]:
    ...
    def get_full_context(
        ...
        if cheap:
            try:
                episodic_context = self.episodic.retrieve_context(
                    current_input, limit=3, cheap=True
                )
            except TypeError:
            ...
            semantic_knowledge = (
                self.semantic.get_relevant_lessons(current_input) if self.semantic else []
            )
