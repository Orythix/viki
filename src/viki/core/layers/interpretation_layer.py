...
class InterpretationLayer(CortexLayer):
    ...
    CLOUD_KEYWORDS = {
        "aws", "ec2", "s3", "lambda", "k8s", "kubernetes", "docker", "terraform",
        "cloud", "instance", "bucket", "cluster",
    }
    DEVOPS_KEYWORDS = {
        "git", "npm", "pip", "pytest", "test", "build", "deploy", "jenkins", "ci", "cd",
        "commit", "branch", "push", "pull", "merge",
    }
    SYSTEM_KEYWORDS = {
        "df", "ps", "top", "free", "mem", "cpu", "disk", "uptime", "whoami",
        "hostname", "ip", "netstat", "memory", "storage", "network",
    }
    DATA_KEYWORDS = {
        "sqlite", "redis", "db", "sql", "table", "data", "database", "query",
        "select", "insert", "update", "delete",
    }
    AI_KEYWORDS = {
        "nvidia", "gpu", "cuda", "torch", "pytorch", "tensorflow", "ollama",
        "transformers", "huggingface", "model", "inference", "train",
    }
    PROD_KEYWORDS = {
        "calendar", "todo", "task", "note", "notes", "agenda", "weather",
        "price", "crypto", "bitcoin", "productivity",
    }
    QUESTION_KEYWORDS = {
    ...
    CORRECTION_KEYWORDS = {
        "wrong", "incorrect", "correction", "fix", "not", "actually", "mistake", "error",
    }
