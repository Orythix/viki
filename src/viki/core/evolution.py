...
class EvolutionEngine:
    ...
    def __init__(self, data_dir: str):
        ...
        self.dynamic_skills_dir = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "skills", "dynamic"
        )
    ...
    def _do_save_mutations(self):
        """Internal method to actually save mutations."""
    ...
    def _save_mutations(self):
        """Debounced save for mutations."""
        self._debouncer.mark_dirty()
        self._debouncer.execute(self._do_save_mutations)
        ...
        for i, m in enumerate(self.mutations["pending"]):
            if m["id"] == m_id:
                ...
                if m["type"] == "reflex":
                    if not self.reflex:
                        viki_logger.warning(
                            f"Evolution: Cannot apply reflex mutation {m_id} - reflex module not set"
                        )
                        return False
                    val = m["value"]
        ...
        for m in self.mutations["pending"]:
            if m.get("pattern_id") == pattern_id:
                ...
                if m["success_count"] >= 3:
                    viki_logger.info(
                        f"Evolution: Mutation {m['id']} has {m['success_count']} successes. Ready for manual approval via /approve {m['id']}"
                    )
    ...
    def get_agent_weightings(self) -> dict[str, float]:
        ...
        weights = {"curiosity": 1.0, "skepticism": 1.0, "efficiency": 1.0, "autonomy": 1.0}
    ...
    def get_evolution_summary(self, limit: int = 10) -> str:
        ...
        if not applied:
            if not self.crystallized_summary:
                return (
                    "Identity Status: Stable. No significant deviations from core priors recorded."
                )
            return summary
    ...
    async def crystallize_identity(self):
        ...
        history_text = "\n".join(
            [f"- {m['description']} (at {m.get('applied_at', 0)})" for m in applied]
        )
        ...
        prompt = [
            ...
            {
                "role": "user",
                "content": f"CURRENT IDENTITY BASE: {self.crystallized_summary}\n\nNEW MUTATIONS:\n{history_text}",
            },
        ]
    ...
    async def propose_skill(self, task_description: str, code_hint: str = ""):
        ...
        prompt = [
            {
                ...
                "content": (
                    ...
                    "The skill must inherit from `BaseSkill` which must be imported as `from skills.base import BaseSkill`. \n"
                    "Structure:\n"
                    "- name: Short unique string (snake_case)\n"
                    "- description: What it does\n"
                    "- schema: Dict for input parameters\n"
                    "- execute: Async method that returns a string result.\n\n"
                    "Ensure ALL necessary imports are included (e.g., `import sys`, `import json`, `from skills.base import BaseSkill`).\n"
        ...
        try:
            ...
            name_match = re.search(
                r"@property.*?def name\(self\).*?return \"(.*?)\"", code, re.DOTALL
            )
                    ...
                    if len(node.args) >= 2:
                        if isinstance(node.args[1], ast.Constant):
                            if node.args[1].value in dangerous_attrs:
                                return (
                                    False,
                                    f"Dangerous attribute access via getattr: {node.args[1].value}",
                                )
