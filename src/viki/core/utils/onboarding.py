...
def _collect_user_details():
    ...
    console.print("[dim]It looks like this is your first time booting VIKI. Let's personalize your sovereign intelligence.[/]\n")
...
def _handle_post_onboarding_training(settings_path, nickname, occupation, more_about_you):
    ...
    if Confirm.ask("\n[bold cyan]Dynamic Training[/] | Would you like VIKI to dynamically learn your profile?"):
        ...
        if Confirm.ask("[bold cyan]Model Bake[/] | Bake this identity into a specialized Ollama model (viki-evolved)?"):
...
def _inject_knowledge(settings_path: str, nickname: str, occupation: str, more_info: str):
    ...
    try:
        ...
        conn.execute("DELETE FROM lessons WHERE author='Self' AND (text_representation LIKE '%Orythix%' OR text_representation LIKE '%Your name is %')")
        conn.execute("DELETE FROM lessons WHERE text_representation LIKE '%Owner Identity%' OR text_representation LIKE '%Owner Occupation%' OR text_representation LIKE '%Owner Background%'")

        facts = [
            (f"Owner Identity", f"The owner's name is {nickname}. Call them {nickname}."),
            (f"Owner Occupation", f"{nickname} works as a {occupation}."),
        ]
        if more_info:
        ...
        for trigger, fact in facts:
            ...
            conn.execute('''INSERT OR REPLACE INTO lessons (id, content, text_representation, created_at, access_count, author)
                           VALUES (?, ?, ?, ?, ?, ?)''', 
...
def _bake_model(settings_path: str, nickname: str, occupation: str, more_info: str):
    ...
    try:
        with console.status("[bold cyan]Baking model...[/] (This takes 30-60s)"):
            ...
            # Run ollama create
            result = subprocess.run(["ollama", "create", "viki-evolved", "-f", modelfile_path], 
