...
class IntelligenceScorecard:
    ...
    def record_metric(self, name: str, value: float, context: str = None, model: str | None = None):
    ...
    def get_summary(self, model: str | None = None) -> dict[str, float]:
    ...
    def get_sparkline_series(
        self, points: int = 30, model: str | None = None
    ) -> dict[str, list[float]]:
        """
    ...
    def get_segmented_trends(
        ...
        for m in sorted(models):
            out[m] = {
                ...
                "regressions": self.detect_regressions(regression_window, regression_threshold, m),
            }
        return out
    ...
    def check_plateau(self, window: int = 20) -> bool:
        ...
        for _name, entries in self.metrics.items():
